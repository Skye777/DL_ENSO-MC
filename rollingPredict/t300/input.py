...
def train_input_fn():
    ...
    train_filenames = [hp.rollingPred + f"/t300/tfRecords/train-in{hp.in_seqlen}.tfrecords"]
    train_dataset = tf.data.TFRecordDataset(train_filenames)
    train_dataset = train_dataset.map(parse_fn)
    train_dataset = train_dataset.shuffle(hp.random_seed).batch(hp.batch_size)

    test_filenames = [hp.rollingPred + f"/t300/tfRecords/test-in{hp.in_seqlen}.tfrecords"]
