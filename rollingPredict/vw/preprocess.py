...
def parse_npz_and_nc_data():
    ...
    for i in range(sst.shape[0] - hp.in_seqlen + 1 - hp.lead_time - hp.out_seqlen):
        data.append({'sst': sst[i:i + hp.in_seqlen].astype(np.float32),
                     ...
                     'taux': taux[i:i + hp.in_seqlen].astype(np.float32),
                     'tauy': tauy[i:i + hp.in_seqlen].astype(np.float32)})

        target_start = i + hp.in_seqlen + hp.lead_time
        target.append({'tauy': tauy[target_start].astype(np.float32)})
...
def write_records(data, filename):
    series = data[0]
    target = data[1]
    writer = tf.io.TFRecordWriter(f"{hp.rollingPred}/vw/tfRecords/{filename}")
    ...
    for s, t in zip(series, target):
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'output_tauy': _bytes_feature(t['tauy'].tobytes())
        }))
...
if __name__ == "__main__":
    ...
    write_records((train_data, train_target), f"train-in{hp.in_seqlen}.tfrecords")
    write_records((test_data, test_target), f"test-in{hp.in_seqlen}.tfrecords")
