...
def get_features():
    features = {
        'input_sst': tf.io.FixedLenFeature([], tf.string), 'input_t300': tf.io.FixedLenFeature([], tf.string),
        'input_taux': tf.io.FixedLenFeature([], tf.string), 'input_tauy': tf.io.FixedLenFeature([], tf.string),
        'output_sst': tf.io.FixedLenFeature([], tf.string), 'output_t300': tf.io.FixedLenFeature([], tf.string),
        'output_taux': tf.io.FixedLenFeature([], tf.string), 'output_tauy': tf.io.FixedLenFeature([], tf.string)}
    return features
...
def train_input_fn():
    ...
    train_filenames = [hp.directRollingPred + f"/tfRecords/train-in{hp.in_seqlen}.tfrecords"]
    train_dataset = tf.data.TFRecordDataset(train_filenames)
    train_dataset = train_dataset.map(parse_fn)
    train_dataset = train_dataset.shuffle(hp.random_seed).batch(hp.batch_size)

    test_filenames = [hp.directRollingPred + f"/tfRecords/test-in{hp.in_seqlen}.tfrecords"]
